...
function showQuestion(question) {
  questionElement.innerText = question.question/*innertext of the question element becomes whatever the questions array outputs*/
  question.answers.forEach(answer => {/*populates the answer buttons by looping through the answers for each of the questions*/
    ...
    be used to remove the original 4 answer buttons*/
  })
}
...
function selectAnswer(e) {/*Takes the answer button click event from above in as a parameter */
  const selectedButton = e.target/*assigns a variable to the selected button using e.target*/
...
//timer--
if(seconds>0){
  timeElement.innerHTML = timeElement-1;
}
}
/*******************************
...
const questions = [
  ...
  },
  {
    ...
    answers: [
      { text: 'Keep up-to-date antivirus and anti-malware', correct: false },
